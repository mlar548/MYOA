				                ...
				                <c:forEach var="role" items="${allRoles}">
				                	<tr id="${role.id}">
				                		...
				                		<td>
											...
											<a href="delRole?delRoleId=${role.id}" onclick="if(confirm('确定删除?')==false)return false;" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-remove"></span> 删除</a>
				                		</td>
				                	</tr>
				                </c:forEach>
	                    </tbody>
...
function viewPermissions(role_id){
	...
	$.ajax({
		...
		type:'post',
		data:{
			roleId:role_id	//对应的是user的主键ID
		},
		dataType:'json',
		success:function(permissionList) {
			var permissionList = permissionList.list
			//$("input[type=checkbox]").attr('checked',false);
			$.each(permissionList,function(i,perm){
				$('#chk'+perm.sysPermissionId).attr('checked',true);
			});
